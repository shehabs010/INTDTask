...
export class AppComponent implements OnInit {
  ...
  title = 'todo-list';
  todos:Array <Todo> = window.localStorage.getItem('todos-List') ? JSON.parse(window.localStorage.getItem('todos-List') || '[]') : [];
  completedTodoList:Array<Todo> = window.localStorage.getItem('todos-Done-List') ? JSON.parse(window.localStorage.getItem('todos-Done-List') || '[]') : [];
  editObject = null;
  endTaskTime= 0;
  ...
  handelEventComplete(serverData:any){
    ...
    window.localStorage.setItem('todos-Done-List', JSON.stringify(this.completedTodoList));
  ...
  handelEventNotComplete(serverDatas:any){
    let unTodoItem = this.completedTodoList.filter(x=> x.id == serverDatas)[0];
  ...
  handelEventDelete(serverDeleteData: any){
    if(serverDeleteData.status){
      let todoItem = this.completedTodoList.filter(x => x.id === serverDeleteData)[0];
...
var m3 = m1.diff(m2,'minutes') * 60;
      ...
      // let dueDateTask = new Date(taskNotification.dueDate).get();
      // let time = dueDateTask - toDay;
     console.log(m3)
     if(m3 > 0){
  ...
  sortByTasks(){
     let sortedTodos = _.sortBy(this.todos, [function(o) { return o.dueDate}]);
     return sortedTodos
  }
  sortByDoneTasks(){
    let sortedDoneTodos = _.sortBy(this.completedTodoList, [function(o) { return o.dueDate}]);
